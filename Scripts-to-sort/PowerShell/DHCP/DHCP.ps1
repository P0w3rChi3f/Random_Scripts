...
Since there is no .NET API, I had to use netsh (with its inherent limitations) to handle everything I wanted to do in DHCP. With that said, this module is not an exhaustive translation of every command available in netsh for DHCP, but the ones I use most often.
...
Note that 7 new structures/data types are created by this module. To understand what attributes each structure has, see the Add-Type section at the top of the script.
 
Please comment on any errors you find, or inconsistencies in verbage, parameters, etc. Also, please offer any ideas you have for enhancements, additions, fixes, etc.

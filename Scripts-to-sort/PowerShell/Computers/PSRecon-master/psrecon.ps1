...
Write-Output "# Get user and admin info"
$whoami = $env:username
qwinsta > PSRecon\config\activeUsers.html
$activeUsersA = Get-Content PSRecon\config\activeUsers.html
$activeUsers = $activeUsersA | ForEach-Object {"$_<br/>"}
$activeUsers > PSRecon\config\activeUsers.html

Write-Output "# Set environmental variables"
...
Write-Output "# Display banner and host data"
$banner
Write-Host ""
Write-Host "$dateTime : Capturing Host Data : $computerName - $ip"

Write-Output "# Get IP Address Details"
$file = "c:\temp\a.html"
$ipconfig=ipconfig /all |ForEach-Object{"$_<br/>"}
$body="<html><head><title>Network Report</title></head><body><h1>IPConfig /All</h1><pre>$ipconfig</pre></body></html>"
$body > PSRecon\config\ipconfig.html
#ipconfig -all > PSRecon\config\ipconfig.html
#$ipconfig = type PSRecon\config\ipconfig.html

Write-Output "# Gathering Scheduled Processes"
$at = at | ForEach-Object {"$_<br/>"}
$at > PSRecon\process\at-jobs.html

Write-Output "# Gathering list of Scheduled Tasks"
$schtasks = Get-ScheduledTask | Where-Object state -EQ 'ready' | Get-ScheduledTaskInfo | Sort-Object TaskPath |Select-Object TaskName, TaskPath | ConvertTo-Html -Fragment

Write-Output "# Extract Installed Hotfix "
$hotfix = get-hotfix | Where-Object {$_.Description -ne ''} | Select-Object -property Description,HotFixID,InstalledBy | foramt-list | ConvertTo-Html -Fragment

Write-Output "# Gathering Process Information"
$taskDetail = tasklist /V /FO CSV | ConvertFrom-Csv | ConvertTo-Html -Fragment

Write-Output "# Gather Windows Service Data"
$serviceDetailA  = Get-WmiObject win32_service | Select-Object Name, DisplayName, PathName, StartName, StartMode, State, TotalSessions, Description | convertto-html
$serviceDetail = $serviceDetailA | ForEach-Object {"$_<br/>"}
$serviceDetail > PSRecon\process\service-detail.html

Write-Output "# DNS Cache"
$dnsCache = Get-DnsClientCache -Status 'Success' | Select-Object Name, Data | ConvertTo-Html -Fragment

Write-Output "# Netstat information"
$netstat = netstat -ant | Select-Object -skip 4 | ConvertFrom-String -PropertyNames none, proto,ipsrc,ipdst,state,state2,none,none | Select-Object ipsrc,ipdst,state | ConvertTo-Html -Fragment

Write-Output "# Display Listening Processes"
$listeningProcesses = netstat -ano | findstr -i listening | ForEach-Object { $_ -split "\s+|\t+" } | findstr /r "^[1-9+]*$" | Sort-Object | Get-unique | ForEach-Object { Get-Process -Id $_ } | Select-Object ProcessName,Path,Company,Description | ConvertTo-Html -Fragment > PSRecon\network\net-processes.html

Write-Output "# ARP table"
$arp = arp -a | Select-Object -skip 3 | ConvertFrom-String -PropertyNames none,IP,MAC,Type | Select-Object IP,MAC,Type | ConvertTo-Html -Fragment

Write-Output "# Gathering information about running services"
$netServices = Get-Service | where-object {$_.Status -eq "Running"} | Select-Object Name, DisplayName | ConvertTo-Html -fragment

Write-Output "#Gathering information about open shares"
net user > PSRecon\system\netuser.html
net use > PSRecon\network\shares.html
$netUserA = get-content PSRecon\system\netuser.html
$netUser = $netUserA | ForEach-Object {$_ + "<br />"}
$sharesA = get-content PSRecon\network\shares.html
$shares = $sharesA | ForEach-Object {$_ + "<br />"}

Write-Output "# Gathering host file information"
...
Write-Output "# Gather Currently Installed Software"
...
Write-Output "# List Recently Used USB Devices"
$usb = Get-ItemProperty -Path HKLM:\SYSTEM\CurrentControlSet\Enum\USBSTOR\*\* | Select-Object FriendlyName | ConvertTo-Html -Fragment > PSRecon\system\usb.html

Write-Output "# Gather command history"
$commandHist = Get-History | ConvertTo-Html -Fragment
# get CMD history with >doskey /history

Write-Output "# Dumping the firewall information"
Write-Output "Firewall State" > PSRecon\system\firewall-config.html
netsh firewall show state >> PSRecon\system\firewall-config.html
Write-Output "Firewall Config" >> PSRecon\system\firewall-config.html
netsh firewall show config >> PSRecon\system\firewall-config.html
Write-Output "Firewall Dump" >> PSRecon\system\firewall-config.html
#netsh dump >> PSRecon\system\firewall-config.html
$firewallA = get-content PSRecon\system\firewall-config.html
$firewall = $firewallA | ForEach-Object {$_ + "<br />"}
$firewall > PSRecon\system\firewall-config.html

Write-Output "# Saving the Environment"
$set = Get-ChildItem ENV: | Select-Object Name, Value | ConvertTo-Html -Fragment

Write-Output "# Return GPResult Output"
& $env:windir\system32\gpresult.exe /v > PSRecon\system\gpresult.html
$gpresultA = get-content PSRecon\system\gpresult.html
$gpresult = $gpresultA | ForEach-Object {$_ + "<br />"}

Write-Output "# Get active SMB sessions"
Get-SmbSession > PSRecon\network\smbsessions.html
$smbSessionA = get-content PSRecon\network\smbsessions.html
$smbSession = $smbSessionS | ForEach-Object {$_ + "<br />"}

Write-Output "# Get ACL's"
$acl = Get-Acl | Select-Object AccessToString, Owner, Group, Sddl | ConvertTo-Html -Fragment

Write-Output "# Gathering Windows version information"
$version = [Environment]::OSVersion | ConvertTo-Html -Fragment

Write-Output "# Dumping the startup information"
Get-Content $env:SystemDrive\autoexec.bat > PSRecon\system\autoexecBat.html 2>&1
Get-Content $env:SystemDrive\config.sys > PSRecon\system\configSys.html 2>&1
Get-Content $env:windir\win.ini > PSRecon\system\winIni.html 2>&1
Get-Content $env:windir\system.ini > PSRecon\system\systemIni.html 2>&1
$autoexecA = get-content PSRecon\system\autoexecBat.html
$autoexec = $autoexecA | ForEach-Object {$_ + "<br />"}
$configSysA = get-content PSRecon\system\configSys.html
$configSys = $ConfigSysA | ForEach-Object {$_ + "<br />"}
$winIniA = get-content PSRecon\system\winIni.html
$winIni = $winIniA | ForEach-Object {$_ + "<br />"}
$systemIniA = get-content PSRecon\system\systemIni.html
$systemIni = $systemIniA | ForEach-Object {$_ + "<br />"}

$psversiontable > PSRecon\config\powershell-version.html
$powershellVersionA = Get-Content PSRecon\config\powershell-version.html
$powershellVersion = $powershellVersionA | ForEach-Object {$_ + "<br />"}

Write-Output "# Startup Drivers"
...
Write-Output "# list downloaded files"
$downloads = Get-ChildItem C:\Users\*\Downloads\* -Recurse | Select-Object Name, CreationTime, LastAccessTime, Attributes | ConvertTo-Html -Fragment
$downloads > PSRecon\web\downloads.html

Write-Output "# Extract Prefetch File Listing"
# script stolen from:
...
Switch -Regex ($pfconf) {
    "[1-3]" {
        ...
        Get-ChildItem $env:windir\Prefetch\*.pf | ForEach-Object {
    ...
    default {
        Write-Output "" >> PSRecon\process\prefetch.html
        Write-Output "Prefetch not enabled on ${env:COMPUTERNAME}" >> PSRecon\process\prefetch.html
        Write-Output "" >> PSRecon\process\prefetch.html
    }
}
$prefetch = Get-Content PSRecon\process\prefetch.html

Write-Output "# Extract Internet Explorer History"
# script stolen from:
# https://richardspowershellblog.wordpress.com/2011/06/29/ie-istory-to-csv/
...
get-iehistory | Select-Object Date, URL | ConvertTo-Html -Fragment > PSRecon\web\ie-history.html
$ieHistory = Get-Content PSRecon\web\ie-history.html

Write-Output "# Take a screenshot of the current desktop"
# script stolen from:
...
Move-Item $path .\PSRecon\config\screenshot.png


Write-Output "# Capture Log and Registry Data using cmdlets from Get-ComputerDetails"
...
if ( $remote -eq $true ) {
    ...
    $RDPconnections = $RDPconnectionsA | ForEach-Object {$_ + "<br />"}
    ...
    $psscripts = $psscriptsA | ForEach-Object {$_ + "<br />"}
    ...
    $4624 = $4624A | ForEach-Object {$_ + "<br />"}
    ...
    $4648 = $4648A | ForEach-Object {$_ + "<br />"}
#>
}

Write-Output "# Extract Email Details"
if(-Not ($email)) {
    Write-Output "<p><strong>emails not extracted...</strong><br /><br />" >> PSRecon\web\email-subjects.html
    Write-Output "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; To extract emails, run PSRecon with the [email] command-line switch:<br /><br />" >> PSRecon\web\email-subjects.html
    Write-Output "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; PS C:\> .\PSRecon.ps1 -email" >> PSRecon\web\email-subjects.html
    Write-Output "<br /><br />" >> PSRecon\web\email-subjects.html
    Write-Output "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; This was skipped because email extraction takes a very long time.<br />" >> PSRecon\web\email-subjects.html
    Write-Output "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; This also closes the user's email client and tends to leave the Outlook process hanging...</strong></p><br />" >> PSRecon\web\email-subjects.html
    Copy-Item PSRecon\web\email-subjects.html PSRecon\web\email-links.html
    ...
    $inbox | Select-Object Body | findstr http > PSRecon\web\email-links.html
    $getEmailLinks = 'PSRecon\web\email-links.html'
    $emailLinkRegex = "([a-zA-Z]{3,})://([\w-]+\.)+[\w-]+(/[\w- ./?%&=]*).*?"
    $emailLinksA = select-string -Path $getEmailLinks -Pattern $emailLinkRegex -AllMatches | ForEach-Object { $_.Matches } | ForEach-Object { $_.Value }
    $emailSubjectsA = Get-Content PSRecon\web\email-subjects.html
    $emailSubjects = $emailSubjectsA | ForEach-Object {$_ + "<br />"}
    $emailLinks = $emailLinksA | ForEach-Object {$_ + "<br />"}
...
if ( Test-Path $profile ) {
    $PSprofileA = Get-Content $profile
    $PSProfile = $PSProfileA | ForEach-Object {$_ + "<br />"}
} else {
    $PSprofile = "<br />No PowerShell Profile File Found:<br /><br />"
}

Write-Output "# Last File Created"
$Nb_day = -7
...
Write-Output "# Potential Dangerous Programs, Scripts, Shortcuts, Office Macros, PDF "
...
#=======================================================================================
Write-Output "# Evidence Verification"
#=======================================================================================
...
Get-Process | Where-Object {-not [string]::IsNullOrEmpty($_.Path)} | Select-Object Path -Unique | Sort-Object | Get-FileHash -Algorithm SHA256 | ConvertTo-Html -Fragment >> PSRecon\process\process-hashes.html
...
Remove-Item PSRecon\config\e-hashes.html -Force
$evidenceHashes = Get-Content PSRecon\config\evidence-hashes.html
